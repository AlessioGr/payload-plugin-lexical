...
export default function RawImageComponent({
    ...
    useEffect(() => {
        ...
        async function loadImageData() {
            ...
            const response = await requests.get(`${serverURL}${api}/${relatedCollection?.slug}/${rawImagePayload.value?.id}`, {
                headers: {
            ...
            const imagePayload = {
                altText: json?.text,
                height:  extraAttributes && extraAttributes.heightOverride ? extraAttributes.heightOverride : json?.height,
                maxWidth: extraAttributes && extraAttributes.widthOverride ? extraAttributes.widthOverride : json?.width,
                src: json?.url,
            };
