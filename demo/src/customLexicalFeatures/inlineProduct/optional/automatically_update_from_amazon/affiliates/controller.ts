...
export async function getUpdatedProductShops(shops: {
  shops: Shop[];
}): Promise<Shop[]> {
  ...
  for (let shop of shops.shops) {
    try {
      ...
      if (
        (shop?.shop?.name as string)?.toLowerCase() === 'amazon' ||
        shop?.shop == '63bb051e53c9a3b0f68c4a16'
      ) {
        newShop = await updateShopAmazon(shop);
      } else if (
        (shop?.shop?.name as string)?.toLowerCase() === 'fanatec' ||
        shop?.shop == '64149c2c8a9e524f8e98f51d'
      ) {
