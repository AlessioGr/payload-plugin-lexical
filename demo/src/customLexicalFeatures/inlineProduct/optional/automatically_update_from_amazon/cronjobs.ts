...
export async function updateAllProducts() {
  ...
  for (let productCollection of allProducts.docs) {
    ...
    getUpdatedProductShops({ shops: productCollection.product.shops } as {
      shops: Shop[];
      ...
      if (
        productCollection?.product?.product_images &&
        productCollection?.product?.product_images.length >= 1
      ) {
        let counter = 0;
        for (const image of productCollection?.product?.product_images) {
          counter++;
          if (image?.product_images?.id) {
            productCollection.product.product_images[0].product_images =
      ...
      console.log(
        'Updating product product>shop with new data:',
        updatedProducts,
      );
      // Update collection:
